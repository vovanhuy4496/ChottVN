...
class Save extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        $ruleData['discount_qty'] = $data['rule_settings']['discount_amount'] * $data['apply_settings']['maximum_times'];
    ...
    protected function addSuccessMessage($model)
    {
        ...
        if ($model->getCouponType() != \Magento\SalesRule\Model\Rule::COUPON_TYPE_NO_COUPON) {
            $message .= '<br/>' .
                __('If you want to use multiple coupon codes with auto generation - you can find these settings in the section "Manage coupon codes".');
        }
        if ($isFreeGift && $model->getSimpleAction() != \Amasty\Promo\Api\Data\GiftRuleInterface::SPENT) {
            $message .= '<br/>' .
                __('Don\'t forget to configure promo banners and product labels to highlight your promotion on the storefront.');
        }
