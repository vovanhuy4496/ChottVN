...
include_once 'FineDiffCopyOp.php';
include_once 'FineDiffReplaceOp.php';
include_once 'FineDiffInsertOp.php';
...
class FineDiff {
    ...
    public static function renderToTextFromOpcodes($from, $opcodes) {
        ob_start();
        $renderText = FineDiff::renderFromOpcodes($from, $opcodes, array('FineDiff','renderToTextFromOpcode'));
        ob_get_clean();
        return $renderText;
    }
    ...
    public static function renderDiffToHTMLFromOpcodes($from, $opcodes) {
        ob_start();
        $renderText = FineDiff::renderFromOpcodes($from, $opcodes, array('\Amasty\AdminActionsLog\lib\FineDiff','renderDiffToHTMLFromOpcode'));
        ob_get_clean();
        return $renderText;
    }
    ...
    public static function renderFromOpcodes($from, $opcodes, $callback) {
        ...
        while ( $opcodes_offset <  $opcodes_len ) {
            ...
            if ( $opcode === 'c' ) { // copy n characters from source
                $renderText .= call_user_func($callback, 'c', $from, $from_offset, $n, '');
                $from_offset += $n;
            }
            else if ( $opcode === 'd' ) { // delete n characters from source
                $renderText .= call_user_func($callback, 'd', $from, $from_offset, $n, '');
                $from_offset += $n;
            }
            else /* if ( $opcode === 'i' ) */ { // insert n characters from opcodes
                $renderText .= call_user_func($callback, 'i', $opcodes, $opcodes_offset + 1, $n);
                $opcodes_offset += 1 + $n;
            }
        }
        return $renderText;
