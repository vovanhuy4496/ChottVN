...
include_once 'HtmlNodeComment.php';
...
class HtmlNode {
	 ...
	 * @param HtmlNode $parent Parent of node, null if none
	 */
	...
	/**
	 * Get top parent
	 * @return HtmlNode Root, null if node has no parent
	 */
	function getRoot() {
		$r = $this->parent;
	...
	/**
	 * Change parent
	 * @param HtmlNode $to New parent, null if none
	 ...
	 * @return HtmlNode Null if not found
	 */
	...
	/**
	 * Get childnode
	 * @param int|HtmlNode $child Index, negative to count from end
	...
	#php5e
		return $this->addChild(new HtmlNodeText($this, $text), $offset);
	}
	...
	#php5e
		return $this->addChild(new HtmlNodeComment($this, $text), $offset);
	}
	...
	#php5e
		return $this->addChild(new HtmlNodeConditional($this, $condition, $hidden), $offset);
	}
	...
	#php5e
		return $this->addChild(new HtmlNodeCdata($this, $text), $offset);
	}
	...
	#php5e
		return $this->addChild(new HtmlNodeDoctype($this, $dtd), $offset);
	}
	...
	#php5e
		return $this->addChild(new HtmlNodeXML($this, $tag, $text, $attributes), $offset);
	}
	...
	#php5e
		return $this->addChild(new HtmlNodeAsp($this, $tag, $text, $attributes), $offset);
	}

	/**
	 * Delete a childnode
	 * @param int|HtmlNode $child Child(index) to delete, negative to count from end
	...
	/**
	 * Wrap node
	 * @param string|HtmlNode $node Wrapping node, string to create new element node
	...
	function select($query = '*', $index = false, $recursive = true, $check_self = false) {
		$s = new HtmlSelector($this, $query, $check_self, $recursive);
		$res = $s->result;
		unset($s);
