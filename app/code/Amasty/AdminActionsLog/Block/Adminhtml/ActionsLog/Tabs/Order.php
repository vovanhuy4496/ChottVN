...
class Order extends \Amasty\AdminActionsLog\Block\Adminhtml\ActionsLog\Tabs\DefaultItemColumns
{
    protected function _prepareCollection()
    {
        ...
        $collection->getSelect()
            ->joinRight(array('r' => $this->_objectManager->get('Amasty\AdminActionsLog\Model\LogDetails')->getCollection()->getMainTable()), 'main_table.id = r.log_id', array('is_logged' => 'MAX(r.log_id)'))
            ->where("element_id = ?", $elementId)
            ->where("category like ?", '%order%')
            ->group('r.log_id')
        ;
