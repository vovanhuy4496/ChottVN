define([
    ...
    return Component.extend({
        defaults: {
            isCreateAccountAction: ko.observable(false),
            ...
            if (this.createAcc === '2') {
                this.validate = ko.computed(function () {
                    $.async('#customer-password', function (elem) {
                        if (this.isPasswordVisible() && this.isCreateAccountAction() === false) {
                    ...
                    return this.isPasswordVisible() && this.isCreateAccountAction() === false;
                }.bind(this));
            }
         ...
         */
        login: function () {
            if (this.createAcc === '2' && this.isPasswordVisible() && this.isCreateAccountAction() === false) {
        ...
        _visibleReCaptcha: function () {
            if (this.mspRecaptcha()) {
                $.async({
                    ...
                    if (this.isPasswordVisible() && this.isCreateAccountAction()) {
                        $(loginWrapper).show();
                    } else {
                        $(loginWrapper).hide();
                    }
