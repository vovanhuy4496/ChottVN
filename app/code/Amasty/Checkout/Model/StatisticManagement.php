...
class StatisticManagement
{
    ...
    private function getGraphicInfo()
    {
        ...
        foreach (DefaultConfigProvider::BLOCK_NAMES as $blockCode => $blockLabel) {
            if ($blockCode === 'block_order_summary') {
                continue;
            }

            $blockInfo = $this->serializer->unserialize($this->config->getBlockInfo($blockCode, $store));
            $this->statisticData['data']['graphic_info'][] = [
                'label' => $blockInfo['value'] ?: (string)__($blockLabel),
    ...
    private function collectDeliveryData()
    {
        ...
        foreach (self::DELIVERY_INFO as $code => $label) {
            ...
            $clone->addFieldToFilter($code, ['neq' => '-']);
        ...
        $this->statisticData['data']['block_delivery_total_count'] = $totalCount->getSize();
    }
    ...
    private function collectPaymentData()
    {
        ...
        $this->statisticData['data']['block_payment_method_total_count'] = $totalCount;
    }
    ...
    private function collectShippingData()
    {
        ...
        $this->statisticData['data']['block_shipping_method_total_count'] = $totalCount;
    }
    ...
    private function collectAddressData()
    {
        ...
        $this->statisticData['data']['block_shipping_address_total_count'] =
