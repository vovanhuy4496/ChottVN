...
class Create extends CreatePost
{
     ...
     * @return Data|mixed
     */
    protected function getPhoneVerification()
    {
        if (!$this->phoneVerificationRepository) {
            $this->phoneVerificationRepository = ObjectManager::getInstance()->get(\Chottvn\SigninPhoneNumber\Api\PhoneVerificationRepositoryInterface::class);
        }

        return $this->phoneVerificationRepository;
    }
    ...
    {
        if (!$this->coreSession) {
            $this->coreSession = ObjectManager::getInstance()->get(\Magento\Framework\Session\SessionManagerInterface::class);
        }
    ...
    public function execute()
    {
        ...
        try {
            ...
            if (!$this->checkPasswordConfirmation($password, $confirmation)) {
                ...
                if(true) { //Condition Validate account with phone number
                    // Send OTP
                    $phoneNumberAttribute = $customer->getCustomAttribute('phone_number');
                    $phoneVerificationRepository = $this->getPhoneVerification();
                    $phoneVerificationRepository->sendOTP($customer->getId());

                    $coreSession = $this->getCoreSession();
                    $coreSession->setSignInPhone($phoneNumberAttribute->getValue());

                    $result['redirect'] = $this->urlModel->getUrl('signinphonenumber/phoneverification/display');

                    $result['success'] = true;
                    $this->messageManager->addSuccess(
                        __(
                            'You must verify your phone number %1. Please check your phone message for the OTP Code.',
                            $phoneNumberAttribute->getValue()
                        )
                    );
                }else{
            ...
            $result['message'][] = __('We can\'t save the customer.') . $e->getMessage();
        }
