...
class Data extends AbstractData
{
    ...
    public function getBannerCollection($id = null)
    {
        ...
        ->addFieldToFilter('start_date', ["to" => $this->date->date()])
        ->addFieldToFilter(
            ['end_date','end_date'],
            [
                ['from' => $this->date->date()],
    ...
    public function getBannerById($id = null)
    {
        ...
        // query by id
        $collection->addFieldToFilter('banner_id', array('eq' => $id));

        // status check enable
        $collection->addFieldToFilter('status', array('eq' => 1));

        // return by first banner item
        return $collection->getFirstItem();
    }

    public function getDataBannerImageHtml($object,$display_mode,$is_allow_show,$css_class,$page){
        $html = '';
        // access or not
        if($is_allow_show){
            // get by display mode
            $target = $object->getNewtab() == 0 ? '_self':'_blank';
            ...
            if($object->getData()){
                switch ($display_mode) {
                    ...
                    case 'link':
                        $html .= '<div class="'.$page.$css_class.'">';
                        $html .= '<a href="'.$url.'" title="'.$title.'" target="'.$target.'">';
                        $html .= '<span>'.$title.'</span>';
                        $html .= '</a>';
                        $html .= '</div>';
                        break;
