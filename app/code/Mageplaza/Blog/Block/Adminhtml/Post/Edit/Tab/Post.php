...
class Post extends Generic implements TabInterface
{
    ...
    protected function _prepareForm()
    {
        ...
        $fieldset->addField('is_video', 'select', [
            'name' => 'is_video',
            'label' => __('Show play icon'),
            'title' => __('Show play icon'),
            'values' => $this->enabledisable->toOptionArray()
            ]
        );
        ...
        $fieldset->addField('catalog_ids', '\Mageplaza\Blog\Block\Adminhtml\Post\Edit\Tab\Renderer\Entity', [
            'name' => 'catalog_ids',
            'label' => __('Categories Entity'),
            'title' => __('Categories Entity'),
            ]
        );
        if (!$post->getCatalogIds()) {
            $post->setCatalogIds($post->getCatalogCaIds());
        }
    ...
    //    var_dump(get_class_methods($post));
        ...
        if ($post->getData('start_date')) {
            $date = $post->getData('start_date');
            $date = new \DateTime($date, new \DateTimeZone('UTC'));
            $date->setTimezone(new \DateTimeZone($this->_localeDate->getConfigTimezone()));
            $post->setData('start_date', $date);
        }
