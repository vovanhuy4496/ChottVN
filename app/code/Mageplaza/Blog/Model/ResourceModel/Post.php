...
class Post extends AbstractDb
{
     ...
     * @var string
     */
    public $catalogcateProductTable;
    ...
    )
    {
        ...
        $this->catalogcateProductTable = $this->getTable('mageplaza_blog_post_category_catalog');
    }
    ...
    protected function _afterSave(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $this->saveCatalogRelation($object);
     ...
     * @throws \Magento\Framework\Exception\LocalizedException
     */
    public function saveCatalogRelation(\Mageplaza\Blog\Model\Post $post)
    {
        $post->setIsChangedCategoryList(false);
        $id = $post->getId();
        $categories = $post->getCatalogIds();
        if ($categories === null) {
            return $this;
        }
        $oldCategoryIds = $post->getCatalogCaIds();
        $insert = array_diff($categories, $oldCategoryIds);
        $delete = array_diff($oldCategoryIds, $categories);
        $adapter = $this->getConnection();
        if (!empty($delete)) {
            $condition = ['entity_id IN(?)' => $delete, 'post_id=?' => $id];
            $adapter->delete($this->catalogcateProductTable, $condition);
        }
        if (!empty($insert)) {
            $data = [];
     ...
     * @return array
     */
    public function getCatalogCaIds(\Mageplaza\Blog\Model\Post $post)
    {
        $adapter = $this->getConnection();
        $select = $adapter->select()->from($this->catalogcateProductTable, 'entity_id')
            ->where('post_id = ?', (int)$post->getId());
        return $adapter->fetchCol($select);
    }
