...
class UpgradeSchema implements UpgradeSchemaInterface
{
    ...
    public function upgrade(SchemaSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        if (version_compare($context->getVersion(), '2.4.5', '<')) {
            if ($installer->tableExists('mageplaza_blog_post')) {
                $connection->addColumn($installer->getTable('mageplaza_blog_post'), 'is_video', [
        ...
        if (version_compare($context->getVersion(), '2.4.6', '<')) {
            if ($installer->tableExists('mageplaza_blog_post')) {
                $connection->addColumn($installer->getTable('mageplaza_blog_post'), 'start_date', [
                    'type' => Table::TYPE_DATETIME, null,
                    'comment' => 'Post Start Date',
                ]);
                $connection->addColumn($installer->getTable('mageplaza_blog_post'), 'end_date', [
                    'type' => Table::TYPE_DATETIME, null,
                    'comment' => 'Post End Date',
                ]);
            }
        }
        if (version_compare($context->getVersion(), '2.4.7', '<')) {
            if (!$installer->tableExists('mageplaza_blog_post_category_catalog')) {
                $table = $installer->getConnection()
                    ->newTable($installer->getTable('mageplaza_blog_post_category_catalog'))
                    ->addColumn('entity_id', Table::TYPE_INTEGER, null, [
                        'unsigned' => true,
                        'primary' => true,
                        'nullable' => false
                    ...
                    ->addColumn('position', Table::TYPE_INTEGER, null, ['nullable' => false, 'default' => '0'], 'Position')
                    ->addIndex($installer->getIdxName('mageplaza_blog_post_category_catalog', ['entity_id']), ['entity_id'])
                    ->addIndex($installer->getIdxName('mageplaza_blog_post_category_catalog', ['post_id']), ['post_id'])
                    ->addForeignKey(
                        $installer->getFkName('mageplaza_blog_post_category_catalog', 'entity_id', 'catalog_category_entity', 'entity_id'),
                        'entity_id',
                    ...
                    ->addForeignKey(
                        $installer->getFkName('mageplaza_blog_post_category_catalog', 'post_id', 'mageplaza_blog_post', 'post_id'),
                        'post_id',
                        $installer->getTable('mageplaza_blog_post'),
                        'post_id',
                        Table::ACTION_CASCADE
                    ...
                    ->addIndex(
                        $installer->getIdxName('mageplaza_blog_post_category_catalog', ['entity_id', 'post_id'], AdapterInterface::INDEX_TYPE_UNIQUE),
                        ['entity_id', 'post_id'],
                        ['type' => AdapterInterface::INDEX_TYPE_UNIQUE]
                    )
                    ->setComment('Category Entity Blog');
