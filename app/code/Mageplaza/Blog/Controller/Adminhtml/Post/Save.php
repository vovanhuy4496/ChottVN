...
class Save extends Post
{
    ...
    public function execute()
    {
        ...
        if ($data = $this->getRequest()->getPost('post')) {
            ...
            if($postId == 0){
                $data['start_date'] = $timezone->convertConfigTimeToUtc(isset($data['start_date']) ? $data['start_date'] : null);
                ...
                if ($this->validateDate($data['start_date'])){
                    $data['start_date'] = $timezone->convertConfigTimeToUtc($data['start_date']);
                    $data['publish_date'] = $timezone->convertConfigTimeToUtc(isset($data['publish_date']) ? $data['publish_date'] : null);
                }else{
                    $data['start_date'] = null;
                    $data['publish_date'] = $timezone->convertConfigTimeToUtc(null);
                }
            }
    ...
    protected function prepareData($post, $data = [])
    {
        ...
        $data['catalog_ids'] = (isset($data['catalog_ids']) && $data['catalog_ids']) ? explode(',', $data['catalog_ids']) : [];
        $post->addData($data);
