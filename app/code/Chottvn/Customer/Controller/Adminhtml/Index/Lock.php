...
/**
 * Lock Customer action.
 */
class Lock extends \Chottvn\Affiliate\Controller\Adminhtml\Index implements HttpPostActionInterface
{
    /**
     * Lock Customer action.
     *
        ...
        if (!empty($customerId)) {
            try {
                ...
                if ($customer) {
                    ...
                    // Send Email
                    if ($checkSendMail == 1) {
                        $this->_affiliateHelper->sendLockedEmail($data);
                    }

                    $this->messageManager->addSuccess(__('You locked the customer.'));
                } else {
    ...
    {
        $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/customer_lock.log');
