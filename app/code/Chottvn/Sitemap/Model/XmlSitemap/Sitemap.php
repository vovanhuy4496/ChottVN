...
namespace Chottvn\Sitemap\Model\XmlSitemap;
...
class Sitemap extends AbstractModel implements SitemapInterface
{
    ...
    private function generateBlog()
    {
        ...
        $helper = \Magento\Framework\App\ObjectManager::getInstance()->get('Mageplaza\Blog\Helper\Data');
        $blogLinks = $helper->getPostList($this->storeId);
        // echo 'aaaa';
        // print_r($values->getSelect()->__toString());
        // print_r(get_class_methods($values));exit;
        foreach ($blogLinks as $link) {
            // echo $link->getUrl();
            // echo '<pre>';print_r(get_class_methods($link));echo '</pre>';exit;
            if ($link->getUrl()) {
                if ($this->_isUrlToExclude($link->getUrl())) {
                    continue;
                }

                $blog_link = $this->isFrontUrlSecure() ? $this->replaceHttpToHttps($link->getUrl()) : $link->getUrl();
                $this->xml[] = sprintf(
                    self::XML_LINE,
                    $blog_link,
                    $changefreq,
