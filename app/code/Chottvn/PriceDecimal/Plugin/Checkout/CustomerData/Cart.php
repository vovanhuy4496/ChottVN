...
namespace Chottvn\PriceDecimal\Plugin\Checkout\CustomerData;
...
class Cart
{
    ...
    public function afterGetSectionData(\Magento\Checkout\CustomerData\Cart $subject, $result)
    {
        ...
        $result['subtotal_excl_tax_new'] = $this->checkoutHelper->formatPrice($this->getSubtotalExclTax());
    ...
    protected function getSubtotalExclTax()
    {
        ...
        foreach($items as $item) {
            $subtotal = $subtotal + ((int)$item->getPrice() * (int)$item->getQty());
        }
        return $subtotal;
    }
    ...
    */
    private function writeLog($info, $type = "info") {
        $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/TotalPrice_Minicart.log');
