...
namespace Chottvn\PriceQuote\Block\Index;
...
class Index extends \Magento\Framework\View\Element\Template
{
     ...
     * @return int
     */
    public function getItemsCount($quoteId)
    {
    ...
    public function getShippingAmount()
    {
        ...
        // $getShippingAmount = $this->quoteTotalShipping->returnShippingAmount();
        // $this->writeLog($this->getShippingAddress()->getShippingAmount());
        // echo '<pre>';print_r($getShippingAmount);echo '</pre>';
        // echo '<pre>';print_r($this->quoteTotalShipping->returnShippingAmount());echo '</pre>';
        if ($shippingAmount > 0) {
    ...
    public function checkWeight()
    {
        ...
        foreach ($items as $item) {
            ...
            // $this->writeLog($rule);
            if ($item->getQty() > 0 && empty($item->getAmpromoRuleId()) && !$rule && !$item->getFreeShipping()) {
    ...
    {
        $grandTotal = $this->getShippingAddress()->getGrandTotal();
        if ($grandTotal == 0){
            $grandTotal = $this->getSubtotal() + $this->getshippingAmount() - $this->getDiscountAmount();
        }
