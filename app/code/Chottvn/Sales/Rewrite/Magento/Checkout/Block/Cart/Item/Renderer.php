 ...
 */
namespace Chottvn\Sales\Rewrite\Magento\Checkout\Block\Cart\Item;
...
{
    // Huy khai bao de lam task 965
    const XML_PATH_STOCK_THRESHOLD_QTY = 'cataloginventory/options/stock_threshold_qty';

    public $getStockItemConfiguration;
    public $productSalableQty;
    ...
    public function checkStatusQtyProduct()
    {
        ...
        $helperbg = $objectManager->create('Chottvn\PriceQuote\Helper\Data');
        /** @var QuoteItem $item */
        foreach ($quote->getAllVisibleItems() as $index => $item) {
            ...
            if($attributesOption){
                ...
                $itemQuoteCollection = $itemQuote->create()->addFieldToFilter('quote_id', $quote->getId())
                ...
                $defaultStockQty = $this->productSalableQty->execute($product_sku, $stockId);
            }

            if ($defaultStockQty == 0) {
                // $out[$index]['status'] = false;
                // $out[$index]['type'] = 1;
                $out[$index]['message'] = __('Product %1 is out of stock. Please check the information.', $product->getData('product_name_distributor'));
            }
            if ($defaultStockQty != 0 && ($defaultStockQty < $item->getQty())) {
                // $out[$index]['status'] = false;
                // $out[$index]['type'] = 2;
                $out[$index]['message'] = __('The product %1 is not of sufficient quantity. Please check the information.', $product->getData('product_name_distributor'));
            }
        }
        // $this->writeLog($out);
