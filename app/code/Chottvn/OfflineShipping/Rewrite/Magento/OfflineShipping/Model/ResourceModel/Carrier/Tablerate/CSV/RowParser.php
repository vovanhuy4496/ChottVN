...
namespace Chottvn\OfflineShipping\Rewrite\Magento\OfflineShipping\Model\ResourceModel\Carrier\Tablerate\CSV;
...
class RowParser extends \Magento\OfflineShipping\Model\ResourceModel\Carrier\Tablerate\CSV\RowParser
{
    ...
    {
        parent::__construct($locationDirectory);
    ...
    public function parse(
        ...
        $max_delivery_dates = $this->getMaxDeliveryDates($rowData, $columnResolver);
        $min_delivery_dates = $this->getMinDeliveryDates($rowData, $columnResolver);
        $rates = [];
        foreach ($regionIds as $regionId) {
            $rates[] = [
                ...
                'price' => $price,
                'max_delivery_dates' => $max_delivery_dates,
                'min_delivery_dates' => $min_delivery_dates
            ];
        }

        return $rates;
    }
    private function getMaxDeliveryDates(array $rowData, ColumnResolver $columnResolver)
    {
        $MaxDeliveryDates = $columnResolver->getColumnValue(ColumnResolver::COLUMN_MAX_DELIVERY_DATES, $rowData);
        if ($MaxDeliveryDates === '') {
            $MaxDeliveryDates = '*';
        }
        return $MaxDeliveryDates;
    }
    private function getMinDeliveryDates(array $rowData, ColumnResolver $columnResolver)
    {
        $MinDeliveryDates = $columnResolver->getColumnValue(ColumnResolver::COLUMN_MIN_DELIVERY_DATES, $rowData);
        if ($MinDeliveryDates === '') {
            $MinDeliveryDates = '*';
        }
        return $MinDeliveryDates;
    }
