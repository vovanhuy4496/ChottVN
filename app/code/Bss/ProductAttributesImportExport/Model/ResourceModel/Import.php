...
class Import
{
    ...
    /**
     * Set source model for attribute
     *
     * @param string $sourceModel
     * @param string $attributeCode
     */
    public function setSourceModel($sourceModel, $attributeCode)
    {
        $updateData['source_model'] = $sourceModel;

        $condition = ["{$this->getTableName('eav_attribute')}.attribute_code = ?" => $attributeCode];
        $this->writeAdapter->update($this->getTableName('eav_attribute'), $updateData, $condition);
    }

    /**
     * Check if group code exist in attribute set
     *
     ...
     * @return string
     */
    public function checkDuplicateGroupCode($groupName, $groupCode, $attributeSetId)
    {
        $groupCodeByName = trim(preg_replace('/[^a-z0-9]+/', '-', strtolower($groupName)), '-');
        $groupCode = empty($groupCode) ? $groupCodeByName : $groupCode;
        $select = $this->readAdapter->select()
            ->from(
                ['attribute_group' => $this->getTableName('eav_attribute_group')],
                ['attribute_group_id']
            )->where(
                'attribute_group.attribute_group_code = :attribute_group_code'
            )->where(
