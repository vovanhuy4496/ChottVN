define([
    ...
    $.widget('mage.townshipUpdater', {
        options: {
            townshipTemplate: '<option value="<%- data.value %>" <% if (data.isSelected) { %>selected="selected"<% } %>>' +
                '<%- data.title %>' +
                '</option>',
            isTownshipRequired: false,
            optionalTownshipAllowed: true,
        ...
        sortSelect: function(sortTownshipList) {
            let options = sortTownshipList.find("option"),
                arr = options.map(function(_, o) { return { t: $(o).text(), v: o.value }; }).get(),
                selected = sortTownshipList.val();
            ...
            sortTownshipList.val(selected); // set value selected
            return;
        },
        ...
        _updateTownship: function(city) {
            ...
            var townshipListContainter = (this.options.townshipListContainter != undefined) ? $(this.options.townshipListContainter) : null,
                townshipInputContainter = (this.options.townshipInputContainter != undefined) ? $(this.options.townshipInputContainter) : null,
                townshipList = $(this.options.townshipListId),
            ...
            if (this.options.townshipJson[city]) {
                ...
                if (this.options.isTownshipRequired) {
                    ...
                    if (this.options.addressType == 'order-shipping-address' && $('#order-shipping_same_as_billing').length) {
                        if ($('#order-shipping_same_as_billing').is(':checked')) {
                            townshipList.prop('disabled', true);
                        } else {
                            townshipList.removeAttr('disabled');
                        }
                    } else {
                        townshipList.removeAttr('disabled');
                    }
                ...
                this.sortSelect(townshipList);

                townshipList.show();
                if (townshipListContainter != null) {
                    townshipListContainter.show();
                }
                townshipInput.hide();
                if (townshipInputContainter != null) {
                    townshipInputContainter.hide();
                }
                label.attr('for', townshipList.attr('id'));
            } else {
                if (!this.options.optionalTownshipAllowed) { //eslint-disable-line max-depth
                    townshipInput.prop('disabled', true);
                }
                requiredLabel.removeClass('required');
                townshipInput.removeClass('required-entry');

                this._removeSelectOptions(townshipList);
                townshipList.removeClass('required-entry').val('').trigger('change').prop('disabled', true).hide();
                if (townshipListContainter != null) {
                    townshipListContainter.hide();
                }
                townshipInput.show();
                if (townshipInputContainter != null) {
                    townshipInputContainter.show();
                }
