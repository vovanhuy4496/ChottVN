define([
    ...
    $.widget('mage.cityUpdater', {
        ...
        sortSelect: function(sortCityList) {
            let options = sortCityList.find("option"),
                arr = options.map(function(_, o) { return { t: $(o).text(), v: o.value }; }).get(),
                selected = sortCityList.val();
            ...
            sortCityList.val(selected); // set value selected
            return;
        },
        ...
        _updateCity: function(region) {
            ...
            var cityListContainter = (this.options.cityListContainter != undefined) ? $(this.options.cityListContainter) : null,
                cityInputContainter = (this.options.cityInputContainter != undefined) ? $(this.options.cityInputContainter) : null,
            ...
            if (this.options.cityJson[region]) {
                ...
                this.sortSelect(cityList);

                cityList.show();
                if (cityListContainter != null) {
                    cityListContainter.show();
                }
                cityInput.hide();
                if (cityInputContainter != null) {
                    cityInputContainter.hide();
                }
                ...
                this._removeSelectOptions(cityList);
                cityList.removeClass('required-entry').val('').trigger('change').prop('disabled', 'disabled').hide();
                if (cityListContainter != null) {
                    cityListContainter.hide();
                }
                cityInput.show();
                if (cityInputContainter != null) {
                    cityInputContainter.show();
                }
