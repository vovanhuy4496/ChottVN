define([
    ...
    return Component.extend({
        defaults: {
            template: 'Amasty_Rules/summary/discount-breakdown-cart',
            rules: false,
        ...
        getDiscountDataFromTotals: function(totals) {
            if (totals.extension_attributes && totals.extension_attributes.amrule_discount_breakdown) {
                totals.extension_attributes.amrule_discount_breakdown.forEach(element => {
                    element.rule_amount = element.rule_amount.replace(" ₫", "đ");
                    element.rule_amount = element.rule_amount.replace("-", "");
                });
